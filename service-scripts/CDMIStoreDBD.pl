...
=head1 CDMI DBD Update

    CDMIStoreDBD [options]

This script stores the specified database definition file in the CDMI database.
...
if (! $cdmi) {
    print "usage: CDMIStoreDBD [options]\n";
} else {
    # Store the DBD.
    $cdmi->InternalizeDBD();
