...
=head1 svr_identical_genomes

    svr_identical_genomes <genome_ids.tbl >genome_data.tbl

For each incoming genome ID, return the genome ID and name of each that has
...
and ID of each other genomes with identical md5 hashes. It can optionally take as input a list
...
if (! $opted) {
    print "usage: svr_identical_genomes [--url=http://...] [--c=N] <input >output\n";
} else {
    ...
    while (my @tuples = ScriptThing::GetBatch($input, undef, $column)) {
        ...
        my $document = $sapServer->genome_data(-data => ['md5'],-ids => [map { $_->[0] } @tuples]);
	    ...
	    if (@$genomeData > 0)
	    {
		my $idenH = $sapServer->genomes_by_md5( -ids => $genomeData, -names => 1);
		my $same = $idenH->{$genomeData->[0]};
		if ($same && (@$same > 0))
		{
		    foreach my $pair (@$same)
		    {
			my($g,$n) = @$pair;
#			if ($g ne $id)        Matt thought we should allow it
			{
			    print join("\t",($line,$n,$g)),"\n";
			}
		    }
