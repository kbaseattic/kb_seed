...
plan('tests' => 3 * @$sample_genomes + 12);
...
foreach my $sample (@$sample_genomes) {
    ...
    $sample->{'results'} = $cdmi->md5s_to_genomes( [ $sample->{'hash'} ] )->{ $sample->{'hash'} };
    is(@{ $sample->{'results'} }, 1, "Only one genome for md5 $sample->{'hash'}");
    is($genomes->{'hash'}->[0], $genomes->{'results'}, "Correct md5 for $sample->{'id'}");
}
