...
er-query-entity-TimeSeries

=head1 SYNOPSIS

er-query-entity-TimeSeries [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity TimeSeries. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-TimeSeries -is id,exact-match-value -a > records
...
er-query-entity-TimeSeries [arguments] > records
...
my $h = $geO->query_entity_TimeSeries(\@qry, \@fields );
