...
er-query-entity-Experiment

=head1 SYNOPSIS

er-query-entity-Experiment [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Experiment. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Experiment -is id,exact-match-value -a > records
...
er-query-entity-Experiment [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Experiment: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Experiment(\@qry, \@fields );
