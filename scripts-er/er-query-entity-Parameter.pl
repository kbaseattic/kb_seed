...
er-query-entity-Parameter

=head1 SYNOPSIS

er-query-entity-Parameter [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Parameter. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Parameter -is id,exact-match-value -a > records
...
er-query-entity-Parameter [arguments] > records
...
my $h = $geO->query_entity_Parameter(\@qry, \@fields );
