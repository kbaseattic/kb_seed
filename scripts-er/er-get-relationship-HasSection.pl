...
er-get-relationship-HasSection

=head1 SYNOPSIS

er-get-relationship-HasSection [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-HasSection -a < ids > table.with.fields.added
...
Usage: er-get-relationship-HasSection [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the ContigChunk entity to return. Field-list is a comma-separated list of 
strings. The following fields are available:
...
our $usage = <<'END';
Usage: get-relationship-HasSection [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the ContigChunk entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_HasSection(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-HasSection: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
