...
er-query-entity-EcNumber

=head1 SYNOPSIS

er-query-entity-EcNumber [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity EcNumber. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-EcNumber -is id,exact-match-value -a > records
...
er-query-entity-EcNumber [arguments] > records
