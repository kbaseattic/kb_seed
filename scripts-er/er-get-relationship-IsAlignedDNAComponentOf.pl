...
er-get-relationship-IsAlignedDNAComponentOf

=head1 SYNOPSIS

er-get-relationship-IsAlignedDNAComponentOf [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsAlignedDNAComponentOf -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsAlignedDNAComponentOf [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the ContigSequence
...
Choose a set of fields from the AlignmentRow entity to return. Field-list is a comma-separated list of 
strings. The following fields are available:
...
my @all_from_fields = ( 'id', 'length' );
my @all_rel_fields = ( 'from_link', 'to_link', 'index_in_concatenation', 'beg_pos_in_parent', 'end_pos_in_parent', 'parent_seq_len', 'beg_pos_aln', 'end_pos_aln', 'kb_feature_id' );
my @all_to_fields = ( 'id', 'row_number', 'row_id', 'row_description', 'n_components', 'beg_pos_aln', 'end_pos_aln', 'md5_of_ungapped_sequence', 'sequence' );
...
our $usage = <<'END';
Usage: get-relationship-IsAlignedDNAComponentOf [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the AlignmentRow entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsAlignedDNAComponentOf(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsAlignedDNAComponentOf: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
