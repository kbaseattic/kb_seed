...
er-query-entity-Contig

=head1 SYNOPSIS

er-query-entity-Contig [--is field,value] [--like field,value] [--op operator,field,value]
...
Example:
    ...
    er-query-entity-Contig -is id,exact-match-value -a > records
...
er-query-entity-Contig [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Contig: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
