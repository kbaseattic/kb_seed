...
er-get-relationship-AreCodonsFor

=head1 SYNOPSIS

er-get-relationship-AreCodonsFor [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-AreCodonsFor -a < ids > table.with.fields.added
...
Usage: er-get-relationship-AreCodonsFor [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the CodonUsage
...
my @all_from_fields = ( 'id', 'frequencies', 'genetic_code', 'type', 'subtype' );
...
our $usage = <<'END';
Usage: get-relationship-AreCodonsFor [arguments] < ids > table.with.fields.added
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_AreCodonsFor(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-AreCodonsFor: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
