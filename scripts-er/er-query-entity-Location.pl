...
er-query-entity-Location

=head1 SYNOPSIS

er-query-entity-Location [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Location. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Location -is id,exact-match-value -a > records
...
er-query-entity-Location [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Location: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Location(\@qry, \@fields );
