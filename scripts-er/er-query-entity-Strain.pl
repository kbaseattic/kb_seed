...
er-query-entity-Strain

=head1 SYNOPSIS

er-query-entity-Strain [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Strain. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Strain -is id,exact-match-value -a > records
...
er-query-entity-Strain [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Strain: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Strain(\@qry, \@fields );
