...
er-query-entity-Variant

=head1 SYNOPSIS

er-query-entity-Variant [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Variant. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Variant -is id,exact-match-value -a > records
...
er-query-entity-Variant [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Variant: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Variant(\@qry, \@fields );
