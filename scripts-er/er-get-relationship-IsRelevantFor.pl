...
er-get-relationship-IsRelevantFor

=head1 SYNOPSIS

er-get-relationship-IsRelevantFor [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsRelevantFor -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsRelevantFor [arguments] < ids > table.with.fields.added
...
our $usage = <<'END';
Usage: get-relationship-IsRelevantFor [arguments] < ids > table.with.fields.added
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsRelevantFor(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsRelevantFor: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
