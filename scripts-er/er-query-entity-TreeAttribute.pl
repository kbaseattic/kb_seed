...
er-query-entity-TreeAttribute

=head1 SYNOPSIS

er-query-entity-TreeAttribute [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity TreeAttribute. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-TreeAttribute -is id,exact-match-value -a > records
...
er-query-entity-TreeAttribute [arguments] > records
...
my $h = $geO->query_entity_TreeAttribute(\@qry, \@fields );
