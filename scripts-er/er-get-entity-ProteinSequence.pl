...
er-get-entity-ProteinSequence

=head1 SYNOPSIS

er-get-entity-ProteinSequence [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-entity-ProteinSequence -a < ids > table.with.fields.added
...
Usage: er-get-entity-ProteinSequence [arguments] < ids > table.with.fields.added
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-get-entity-ProteinSequence: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
} else {
    print STDERR $usage;
