...
er-query-entity-CompoundInstance

=head1 SYNOPSIS

er-query-entity-CompoundInstance [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity CompoundInstance. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-CompoundInstance -is id,exact-match-value -a > records
...
er-query-entity-CompoundInstance [arguments] > records
...
my $h = $geO->query_entity_CompoundInstance(\@qry, \@fields );
