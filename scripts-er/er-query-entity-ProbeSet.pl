...
er-query-entity-ProbeSet

=head1 SYNOPSIS

er-query-entity-ProbeSet [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity ProbeSet. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-ProbeSet -is id,exact-match-value -a > records
...
er-query-entity-ProbeSet [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-ProbeSet: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_ProbeSet(\@qry, \@fields );
