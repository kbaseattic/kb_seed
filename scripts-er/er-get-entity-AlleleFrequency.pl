...
er-get-entity-AlleleFrequency

=head1 SYNOPSIS

er-get-entity-AlleleFrequency [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-entity-AlleleFrequency -a < ids > table.with.fields.added
...
Usage: er-get-entity-AlleleFrequency [arguments] < ids > table.with.fields.added
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-get-entity-AlleleFrequency: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
} else {
    print STDERR $usage;
