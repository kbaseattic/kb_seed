...
er-get-relationship-IsCollectedInto

=head1 SYNOPSIS

er-get-relationship-IsCollectedInto [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsCollectedInto -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsCollectedInto [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the OTU entity to return. Field-list is a comma-separated list of 
strings. The following fields are available:
...
my @all_rel_fields = ( 'from_link', 'to_link', 'representative' );
my @all_to_fields = ( 'id',  );
...
our $usage = <<'END';
Usage: get-relationship-IsCollectedInto [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the OTU entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsCollectedInto(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsCollectedInto: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
