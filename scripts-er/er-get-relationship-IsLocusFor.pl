...
er-get-relationship-IsLocusFor

=head1 SYNOPSIS

er-get-relationship-IsLocusFor [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsLocusFor -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsLocusFor [arguments] < ids > table.with.fields.added
...
our $usage = <<'END';
Usage: get-relationship-IsLocusFor [arguments] < ids > table.with.fields.added
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsLocusFor: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
