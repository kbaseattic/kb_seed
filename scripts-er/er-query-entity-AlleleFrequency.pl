...
er-query-entity-AlleleFrequency

=head1 SYNOPSIS

er-query-entity-AlleleFrequency [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity AlleleFrequency. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-AlleleFrequency -is id,exact-match-value -a > records
...
er-query-entity-AlleleFrequency [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-AlleleFrequency: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_AlleleFrequency(\@qry, \@fields );
