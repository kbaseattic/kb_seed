...
er-get-relationship-OrdersExperimentalUnit

=head1 SYNOPSIS

er-get-relationship-OrdersExperimentalUnit [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-OrdersExperimentalUnit -a < ids > table.with.fields.added
...
Usage: er-get-relationship-OrdersExperimentalUnit [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the TimeSeries
...
my @all_from_fields = ( 'id', 'source_id', 'name', 'comments', 'timeUnits' );
my @all_rel_fields = ( 'from_link', 'to_link', 'time', 'timeMeta' );
...
our $usage = <<'END';
Usage: get-relationship-OrdersExperimentalUnit [arguments] < ids > table.with.fields.added
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_OrdersExperimentalUnit(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-OrdersExperimentalUnit: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
