...
er-query-entity-TreeNodeAttribute

=head1 SYNOPSIS

er-query-entity-TreeNodeAttribute [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity TreeNodeAttribute. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-TreeNodeAttribute -is id,exact-match-value -a > records
...
er-query-entity-TreeNodeAttribute [arguments] > records
...
my $h = $geO->query_entity_TreeNodeAttribute(\@qry, \@fields );
