...
er-query-entity-ExperimentMeta

=head1 SYNOPSIS

er-query-entity-ExperimentMeta [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity ExperimentMeta. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-ExperimentMeta -is id,exact-match-value -a > records
...
er-query-entity-ExperimentMeta [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-ExperimentMeta: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_ExperimentMeta(\@qry, \@fields );
