...
er-get-relationship-IsSummarizedBy

=head1 SYNOPSIS

er-get-relationship-IsSummarizedBy [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsSummarizedBy -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsSummarizedBy [arguments] < ids > table.with.fields.added
...
my @all_to_fields = ( 'id', 'source_id', 'position', 'minor_AF', 'minor_allele', 'major_AF', 'major_allele', 'obs_unit_count' );
...
our $usage = <<'END';
Usage: get-relationship-IsSummarizedBy [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the AlleleFrequency entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsSummarizedBy(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsSummarizedBy: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
