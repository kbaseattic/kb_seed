...
er-get-relationship-IsModifiedToBuildTree

=head1 SYNOPSIS

er-get-relationship-IsModifiedToBuildTree [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsModifiedToBuildTree -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsModifiedToBuildTree [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the Tree
...
Choose a set of fields from the Tree entity to return. Field-list is a comma-separated list of 
strings. The following fields are available:
...
my @all_from_fields = ( 'id', 'status', 'data_type', 'timestamp', 'method', 'parameters', 'protocol', 'source_id', 'newick' );
my @all_rel_fields = ( 'from_link', 'to_link', 'modification_type', 'modification_value' );
my @all_to_fields = ( 'id', 'status', 'data_type', 'timestamp', 'method', 'parameters', 'protocol', 'source_id', 'newick' );
...
our $usage = <<'END';
Usage: get-relationship-IsModifiedToBuildTree [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the Tree entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsModifiedToBuildTree(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsModifiedToBuildTree: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
