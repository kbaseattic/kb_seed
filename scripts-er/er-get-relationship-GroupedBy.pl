...
er-get-relationship-GroupedBy

=head1 SYNOPSIS

er-get-relationship-GroupedBy [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-GroupedBy -a < ids > table.with.fields.added
...
Usage: er-get-relationship-GroupedBy [arguments] < ids > table.with.fields.added
...
my @all_to_fields = ( 'id', 'source_id', 'name', 'comments', 'groupType' );
...
our $usage = <<'END';
Usage: get-relationship-GroupedBy [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the ExperimentalUnitGroup entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_GroupedBy(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-GroupedBy: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
