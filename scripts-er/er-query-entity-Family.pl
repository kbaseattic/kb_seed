...
er-query-entity-Family

=head1 SYNOPSIS

er-query-entity-Family [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Family. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Family -is id,exact-match-value -a > records
...
er-query-entity-Family [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Family: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
