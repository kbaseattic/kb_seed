...
er-query-entity-Genome

=head1 SYNOPSIS

er-query-entity-Genome [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Genome. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Genome -is id,exact-match-value -a > records
...
er-query-entity-Genome [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Genome: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
