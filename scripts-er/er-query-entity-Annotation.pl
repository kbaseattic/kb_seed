...
er-query-entity-Annotation

=head1 SYNOPSIS

er-query-entity-Annotation [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Annotation. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Annotation -is id,exact-match-value -a > records
...
er-query-entity-Annotation [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Annotation: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
