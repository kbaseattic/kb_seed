...
er-query-entity-StudyExperiment

=head1 SYNOPSIS

er-query-entity-StudyExperiment [--is field,value] [--like field,value] [--op operator,field,value]
...
Example:
    ...
    er-query-entity-StudyExperiment -is id,exact-match-value -a > records
...
er-query-entity-StudyExperiment [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-StudyExperiment: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_StudyExperiment(\@qry, \@fields );
