...
er-query-entity-Attribute

=head1 SYNOPSIS

er-query-entity-Attribute [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Attribute. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Attribute -is id,exact-match-value -a > records
...
er-query-entity-Attribute [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Attribute: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Attribute(\@qry, \@fields );
