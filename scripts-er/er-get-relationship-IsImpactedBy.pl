...
er-get-relationship-IsImpactedBy

=head1 SYNOPSIS

er-get-relationship-IsImpactedBy [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsImpactedBy -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsImpactedBy [arguments] < ids > table.with.fields.added
...
my @all_rel_fields = ( 'from_link', 'to_link', 'source_name', 'rank', 'pvalue', 'position' );
my @all_to_fields = ( 'id', 'trait_name', 'unit_of_measure', 'TO_ID', 'protocol' );
...
our $usage = <<'END';
Usage: get-relationship-IsImpactedBy [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the Trait entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsImpactedBy(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsImpactedBy: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
