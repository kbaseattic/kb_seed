...
er-query-entity-Compound

=head1 SYNOPSIS

er-query-entity-Compound [--is field,value] [--like field,value] [--op operator,field,value]
...
Example:
    ...
    er-query-entity-Compound -is id,exact-match-value -a > records
...
er-query-entity-Compound [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Compound: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
