...
er-get-relationship-IsAssayedBy

=head1 SYNOPSIS

er-get-relationship-IsAssayedBy [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsAssayedBy -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsAssayedBy [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the StudyExperiment
...
my @all_from_fields = ( 'id', 'source_name', 'design', 'originator' );
my @all_rel_fields = ( 'from_link', 'to_link',  );
my @all_to_fields = ( 'id', 'source_id', 'assay_type', 'assay_type_id' );
...
our $usage = <<'END';
Usage: get-relationship-IsAssayedBy [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the Assay entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsAssayedBy(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsAssayedBy: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
