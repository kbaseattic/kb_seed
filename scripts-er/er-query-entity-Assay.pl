...
er-query-entity-Assay

=head1 SYNOPSIS

er-query-entity-Assay [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Assay. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Assay -is id,exact-match-value -a > records
...
er-query-entity-Assay [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Assay: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Assay(\@qry, \@fields );
