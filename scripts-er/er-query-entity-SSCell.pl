...
er-query-entity-SSCell

=head1 SYNOPSIS

er-query-entity-SSCell [--is field,value] [--like field,value] [--op operator,field,value]
...
Example:
    ...
    er-query-entity-SSCell -is id,exact-match-value -a > records
...
er-query-entity-SSCell [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-SSCell: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
