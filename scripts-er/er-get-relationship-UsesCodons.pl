...
er-get-relationship-UsesCodons

=head1 SYNOPSIS

er-get-relationship-UsesCodons [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-UsesCodons -a < ids > table.with.fields.added
...
Usage: er-get-relationship-UsesCodons [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the CodonUsage entity to return. Field-list is a comma-separated list of 
strings. The following fields are available:
...
my @all_to_fields = ( 'id', 'frequencies', 'genetic_code', 'type', 'subtype' );
...
our $usage = <<'END';
Usage: get-relationship-UsesCodons [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the CodonUsage entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_UsesCodons(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-UsesCodons: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
