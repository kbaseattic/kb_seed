...
er-get-relationship-IsDescribedBy

=head1 SYNOPSIS

er-get-relationship-IsDescribedBy [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsDescribedBy -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsDescribedBy [arguments] < ids > table.with.fields.added
...
my @all_to_fields = ( 'id', 'version', 'curator', 'notes', 'description', 'usable', 'private', 'cluster_based', 'experimental' );
...
our $usage = <<'END';
Usage: get-relationship-IsDescribedBy [arguments] < ids > table.with.fields.added
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsDescribedBy(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsDescribedBy: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
