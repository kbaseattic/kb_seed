...
er-query-entity-ContigSequence

=head1 SYNOPSIS

er-query-entity-ContigSequence [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity ContigSequence. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-ContigSequence -is id,exact-match-value -a > records
...
er-query-entity-ContigSequence [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-ContigSequence: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_ContigSequence(\@qry, \@fields );
