...
er-query-entity-Model

=head1 SYNOPSIS

er-query-entity-Model [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Model. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Model -is id,exact-match-value -a > records
...
er-query-entity-Model [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Model: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Model(\@qry, \@fields );
