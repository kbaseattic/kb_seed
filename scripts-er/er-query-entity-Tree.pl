...
er-query-entity-Tree

=head1 SYNOPSIS

er-query-entity-Tree [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Tree. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Tree -is id,exact-match-value -a > records
...
er-query-entity-Tree [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Tree: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Tree(\@qry, \@fields );
