...
er-query-entity-Environment

=head1 SYNOPSIS

er-query-entity-Environment [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Environment. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Environment -is id,exact-match-value -a > records
...
er-query-entity-Environment [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Environment: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Environment(\@qry, \@fields );
