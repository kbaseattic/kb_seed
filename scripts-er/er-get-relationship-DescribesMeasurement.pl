...
er-get-relationship-DescribesMeasurement

=head1 SYNOPSIS

er-get-relationship-DescribesMeasurement [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-DescribesMeasurement -a < ids > table.with.fields.added
...
Usage: er-get-relationship-DescribesMeasurement [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the MeasurementDescription
...
Choose a set of fields from the Measurement entity to return. Field-list is a comma-separated list of 
strings. The following fields are available:
...
my @all_from_fields = ( 'id', 'name', 'description', 'unitOfMeasure', 'category', 'source_id' );
my @all_rel_fields = ( 'from_link', 'to_link',  );
my @all_to_fields = ( 'id', 'source_id', 'value', 'mean', 'median', 'stddev', 'N', 'p_value', 'Z_score' );
...
our $usage = <<'END';
Usage: get-relationship-DescribesMeasurement [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the Measurement entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_DescribesMeasurement(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-DescribesMeasurement: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
