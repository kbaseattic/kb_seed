...
er-get-relationship-ConsistsOfCompounds

=head1 SYNOPSIS

er-get-relationship-ConsistsOfCompounds [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-ConsistsOfCompounds -a < ids > table.with.fields.added
...
Usage: er-get-relationship-ConsistsOfCompounds [arguments] < ids > table.with.fields.added
...
my @all_rel_fields = ( 'from_link', 'to_link', 'molar_ratio' );
...
our $usage = <<'END';
Usage: get-relationship-ConsistsOfCompounds [arguments] < ids > table.with.fields.added
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_ConsistsOfCompounds(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-ConsistsOfCompounds: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
