...
er-query-entity-TaxonomicGrouping

=head1 SYNOPSIS

er-query-entity-TaxonomicGrouping [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity TaxonomicGrouping. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-TaxonomicGrouping -is id,exact-match-value -a > records
...
er-query-entity-TaxonomicGrouping [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-TaxonomicGrouping: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_TaxonomicGrouping(\@qry, \@fields );
