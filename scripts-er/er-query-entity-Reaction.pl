...
er-query-entity-Reaction

=head1 SYNOPSIS

er-query-entity-Reaction [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Reaction. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Reaction -is id,exact-match-value -a > records
...
er-query-entity-Reaction [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Reaction: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Reaction(\@qry, \@fields );
