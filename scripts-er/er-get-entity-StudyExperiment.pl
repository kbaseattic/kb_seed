...
er-get-entity-StudyExperiment

=head1 SYNOPSIS

er-get-entity-StudyExperiment [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-entity-StudyExperiment -a < ids > table.with.fields.added
...
Usage: er-get-entity-StudyExperiment [arguments] < ids > table.with.fields.added
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-get-entity-StudyExperiment: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
} else {
    print STDERR $usage;
