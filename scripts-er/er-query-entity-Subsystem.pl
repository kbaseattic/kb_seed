...
er-query-entity-Subsystem

=head1 SYNOPSIS

er-query-entity-Subsystem [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Subsystem. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Subsystem -is id,exact-match-value -a > records
...
er-query-entity-Subsystem [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Subsystem: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Subsystem(\@qry, \@fields );
