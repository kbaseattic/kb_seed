...
er-query-entity-ProteinSequence

=head1 SYNOPSIS

er-query-entity-ProteinSequence [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity ProteinSequence. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-ProteinSequence -is id,exact-match-value -a > records
...
er-query-entity-ProteinSequence [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-ProteinSequence: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
