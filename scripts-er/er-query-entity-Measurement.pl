...
er-query-entity-Measurement

=head1 SYNOPSIS

er-query-entity-Measurement [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Measurement. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Measurement -is id,exact-match-value -a > records
...
er-query-entity-Measurement [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Measurement: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Measurement(\@qry, \@fields );
