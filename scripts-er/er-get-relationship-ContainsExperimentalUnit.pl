...
er-get-relationship-ContainsExperimentalUnit

=head1 SYNOPSIS

er-get-relationship-ContainsExperimentalUnit [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-ContainsExperimentalUnit -a < ids > table.with.fields.added
...
Usage: er-get-relationship-ContainsExperimentalUnit [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the ExperimentalUnitGroup
...
Choose a set of fields from the ExperimentalUnit entity to return. Field-list is a comma-separated list of 
strings. The following fields are available:
...
my @all_from_fields = ( 'id', 'source_id', 'name', 'comments', 'groupType' );
my @all_rel_fields = ( 'from_link', 'to_link', 'location', 'groupMeta' );
...
our $usage = <<'END';
Usage: get-relationship-ContainsExperimentalUnit [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the ExperimentalUnit entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_ContainsExperimentalUnit(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-ContainsExperimentalUnit: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
