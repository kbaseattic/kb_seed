...
er-get-relationship-IsComprisedOf

=head1 SYNOPSIS

er-get-relationship-IsComprisedOf [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsComprisedOf -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsComprisedOf [arguments] < ids > table.with.fields.added
...
our $usage = <<'END';
Usage: get-relationship-IsComprisedOf [arguments] < ids > table.with.fields.added
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsComprisedOf: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
