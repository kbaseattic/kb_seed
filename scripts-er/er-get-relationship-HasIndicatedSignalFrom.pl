...
er-get-relationship-HasIndicatedSignalFrom

=head1 SYNOPSIS

er-get-relationship-HasIndicatedSignalFrom [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-HasIndicatedSignalFrom -a < ids > table.with.fields.added
...
Usage: er-get-relationship-HasIndicatedSignalFrom [arguments] < ids > table.with.fields.added
...
my @all_rel_fields = ( 'from_link', 'to_link', 'rma_value', 'level' );
...
our $usage = <<'END';
Usage: get-relationship-HasIndicatedSignalFrom [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the Experiment entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_HasIndicatedSignalFrom(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-HasIndicatedSignalFrom: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
