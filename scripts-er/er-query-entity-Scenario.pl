...
er-query-entity-Scenario

=head1 SYNOPSIS

er-query-entity-Scenario [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Scenario. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Scenario -is id,exact-match-value -a > records
...
er-query-entity-Scenario [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Scenario: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Scenario(\@qry, \@fields );
