...
er-query-entity-Locality

=head1 SYNOPSIS

er-query-entity-Locality [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Locality. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Locality -is id,exact-match-value -a > records
...
er-query-entity-Locality [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Locality: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Locality(\@qry, \@fields );
