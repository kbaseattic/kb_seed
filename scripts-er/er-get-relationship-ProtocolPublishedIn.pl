...
er-get-relationship-ProtocolPublishedIn

=head1 SYNOPSIS

er-get-relationship-ProtocolPublishedIn [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-ProtocolPublishedIn -a < ids > table.with.fields.added
...
Usage: er-get-relationship-ProtocolPublishedIn [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the Publication entity to return. Field-list is a comma-separated list of 
strings. The following fields are available:
...
my @all_to_fields = ( 'id', 'title', 'link', 'pubdate' );
...
our $usage = <<'END';
Usage: get-relationship-ProtocolPublishedIn [arguments] < ids > table.with.fields.added
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_ProtocolPublishedIn(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-ProtocolPublishedIn: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
