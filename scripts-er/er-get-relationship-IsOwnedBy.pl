...
er-get-relationship-IsOwnedBy

=head1 SYNOPSIS

er-get-relationship-IsOwnedBy [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsOwnedBy -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsOwnedBy [arguments] < ids > table.with.fields.added
...
our $usage = <<'END';
Usage: get-relationship-IsOwnedBy [arguments] < ids > table.with.fields.added
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsOwnedBy: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
