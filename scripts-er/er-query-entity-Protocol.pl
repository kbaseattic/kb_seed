...
er-query-entity-Protocol

=head1 SYNOPSIS

er-query-entity-Protocol [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Protocol. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Protocol -is id,exact-match-value -a > records
...
er-query-entity-Protocol [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Protocol: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Protocol(\@qry, \@fields );
