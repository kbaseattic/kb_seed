...
er-query-entity-Diagram

=head1 SYNOPSIS

er-query-entity-Diagram [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Diagram. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Diagram -is id,exact-match-value -a > records
...
er-query-entity-Diagram [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Diagram: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Diagram(\@qry, \@fields );
