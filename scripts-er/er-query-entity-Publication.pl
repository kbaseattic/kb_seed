...
er-query-entity-Publication

=head1 SYNOPSIS

er-query-entity-Publication [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Publication. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Publication -is id,exact-match-value -a > records
...
er-query-entity-Publication [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Publication: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
