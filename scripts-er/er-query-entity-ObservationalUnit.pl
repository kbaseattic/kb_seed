...
er-query-entity-ObservationalUnit

=head1 SYNOPSIS

er-query-entity-ObservationalUnit [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity ObservationalUnit. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-ObservationalUnit -is id,exact-match-value -a > records
...
er-query-entity-ObservationalUnit [arguments] > records
...
my $h = $geO->query_entity_ObservationalUnit(\@qry, \@fields );
