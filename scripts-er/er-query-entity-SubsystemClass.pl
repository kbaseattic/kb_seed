...
er-query-entity-SubsystemClass

=head1 SYNOPSIS

er-query-entity-SubsystemClass [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity SubsystemClass. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-SubsystemClass -is id,exact-match-value -a > records
...
er-query-entity-SubsystemClass [arguments] > records
...
my $h = $geO->query_entity_SubsystemClass(\@qry, \@fields );
