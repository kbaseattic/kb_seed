...
er-get-relationship-DescribesAlignment

=head1 SYNOPSIS

er-get-relationship-DescribesAlignment [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-DescribesAlignment -a < ids > table.with.fields.added
...
Usage: er-get-relationship-DescribesAlignment [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the AlignmentAttribute
...
our $usage = <<'END';
Usage: get-relationship-DescribesAlignment [arguments] < ids > table.with.fields.added
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_DescribesAlignment(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-DescribesAlignment: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
