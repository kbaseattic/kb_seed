...
er-get-relationship-IsLocated

=head1 SYNOPSIS

er-get-relationship-IsLocated [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsLocated -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsLocated [arguments] < ids > table.with.fields.added
...
my @all_to_fields = ( 'id', 'source_name', 'city', 'state', 'country', 'origcty', 'elevation', 'latitude', 'longitude', 'lo_accession' );
...
our $usage = <<'END';
Usage: get-relationship-IsLocated [arguments] < ids > table.with.fields.added
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsLocated(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsLocated: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
