...
er-get-relationship-IsAssayOf

=head1 SYNOPSIS

er-get-relationship-IsAssayOf [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsAssayOf -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsAssayOf [arguments] < ids > table.with.fields.added
...
my @all_from_fields = ( 'id', 'source_id', 'assay_type', 'assay_type_id' );
...
our $usage = <<'END';
Usage: get-relationship-IsAssayOf [arguments] < ids > table.with.fields.added
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsAssayOf(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsAssayOf: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
