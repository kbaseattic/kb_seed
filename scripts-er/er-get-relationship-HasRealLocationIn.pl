...
er-get-relationship-HasRealLocationIn

=head1 SYNOPSIS

er-get-relationship-HasRealLocationIn [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-HasRealLocationIn -a < ids > table.with.fields.added
...
Usage: er-get-relationship-HasRealLocationIn [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the CompoundInstance
...
our $usage = <<'END';
Usage: get-relationship-HasRealLocationIn [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the LocationInstance entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_HasRealLocationIn(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-HasRealLocationIn: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
