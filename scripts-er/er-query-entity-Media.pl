...
er-query-entity-Media

=head1 SYNOPSIS

er-query-entity-Media [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Media. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Media -is id,exact-match-value -a > records
...
er-query-entity-Media [arguments] > records
...
my $h = $geO->query_entity_Media(\@qry, \@fields );
