use strict;
use Data::Dumper;
use Carp;

#
# This is a SAS Component
#


=head1 NAME

er-all-entities-ObservationalUnit

=head1 SYNOPSIS

er-all-entities-ObservationalUnit [-a] [--fields fieldlist] > entity-data

=head1 DESCRIPTION

Return all instances of the ObservationalUnit entity.

An ObservationalUnit is an individual plant that 1) is part of an experiment or study, 2) has measured traits, and 3) is assayed for the purpose of determining alleles.  

Example:

    er-all-entities-ObservationalUnit -a 

would retrieve all entities of type ObservationalUnit and include all fields
in the entities in the output.

=head2 Related entities

The ObservationalUnit entity has the following relationship links:

=over 4
    
=item DefinedBy TaxonomicGrouping

=item HasTrait Trait

=item IsLocated Locality

=item IsPartOf StudyExperiment

=item IsVariedIn Contig

=item UsesReference Genome


=back

=head1 COMMAND-LINE OPTIONS

Usage: er-all-entities-ObservationalUnit [arguments] > entity.data

    --fields list   Choose a set of fields to return. List is a comma-separated list of strings.
    -a		    Return all available fields.
    --show-fields   List the available fields.

The following fields are available:

=over 4    

=item source_name

Name/ID by which the observational unit may be known by the originator and is used in queries.

=item source_name2

Secondary name/ID by which the observational unit may be known and is queried.

=item plant_id

ID of the plant that was tested to produce this observational unit. Observational units with the same plant ID are different assays of a single physical organism.


=back

=head1 AUTHORS

L<The SEED Project|http://www.theseed.org>

=cut

use Bio::KBase::CDMI::CDMIClient;
use Getopt::Long;

#Default fields

my @all_fields = ( 'source_name', 'source_name2', 'plant_id' );
my %all_fields = map { $_ => 1 } @all_fields;

our $usage = <<'END';
Usage: er-all-entities-ObservationalUnit [arguments] > entity.data

    --fields list   Choose a set of fields to return. List is a comma-separated list of strings.
    -a		    Return all available fields.
    --show-fields   List the available fields.

The following fields are available:

    source_name
        Name/ID by which the observational unit may be known by the originator and is used in queries.
    source_name2
        Secondary name/ID by which the observational unit may be known and is queried.
    plant_id
        ID of the plant that was tested to produce this observational unit. Observational units with the same plant ID are different assays of a single physical organism.
END


my $a;
my $f;
my @fields;
my $show_fields;
my $help;
my $geO = Bio::KBase::CDMI::CDMIClient->new_get_entity_for_script("a" 		=> \$a,
								  "show-fields" => \$show_fields,
								  "h" 		=> \$help,
								  "fields=s"    => \$f);

if ($help)
{
    print $usage;
    exit 0;
}

if ($show_fields)
{
    print "Available fields:\n";
    print "\t$_\n" foreach @all_fields;
    exit 0;
}

if (@ARGV != 0 || ($a && $f))
{
    print STDERR $usage, "\n";
    exit 1;
}

if ($a)
{
    @fields = @all_fields;
}
elsif ($f) {
    my @err;
    for my $field (split(",", $f))
    {
	if (!$all_fields{$field})
	{
	    push(@err, $field);
	}
	else
	{
	    push(@fields, $field);
	}
    }
    if (@err)
    {
	print STDERR "er-all-entities-ObservationalUnit: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}

my $start = 0;
my $count = 1_000_000;

my $h = $geO->all_entities_ObservationalUnit($start, $count, \@fields );

while (%$h)
{
    while (my($k, $v) = each %$h)
    {
	print join("\t", $k, map { ref($_) eq 'ARRAY' ? join(",", @$_) : $_ } @$v{@fields}), "\n";
    }
    $start += $count;
    $h = $geO->all_entities_ObservationalUnit($start, $count, \@fields);
}
