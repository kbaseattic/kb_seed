...
er-get-relationship-ContainsAlignedDNA

=head1 SYNOPSIS

er-get-relationship-ContainsAlignedDNA [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-ContainsAlignedDNA -a < ids > table.with.fields.added
...
Usage: er-get-relationship-ContainsAlignedDNA [arguments] < ids > table.with.fields.added
...
my @all_from_fields = ( 'id', 'row_number', 'row_id', 'row_description', 'n_components', 'beg_pos_aln', 'end_pos_aln', 'md5_of_ungapped_sequence', 'sequence' );
...
our $usage = <<'END';
Usage: get-relationship-ContainsAlignedDNA [arguments] < ids > table.with.fields.added
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_ContainsAlignedDNA(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-ContainsAlignedDNA: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
