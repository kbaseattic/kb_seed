...
er-query-entity-Feature

=head1 SYNOPSIS

er-query-entity-Feature [--is field,value] [--like field,value] [--op operator,field,value]
...
Example:
    ...
    er-query-entity-Feature -is id,exact-match-value -a > records
...
er-query-entity-Feature [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Feature: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
