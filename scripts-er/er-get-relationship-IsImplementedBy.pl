...
er-get-relationship-IsImplementedBy

=head1 SYNOPSIS

er-get-relationship-IsImplementedBy [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsImplementedBy -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsImplementedBy [arguments] < ids > table.with.fields.added
...
my @all_from_fields = ( 'id', 'role_rule', 'code', 'type', 'comment' );
my @all_rel_fields = ( 'from_link', 'to_link',  );
my @all_to_fields = ( 'id', 'curated', 'region' );
...
our $usage = <<'END';
Usage: get-relationship-IsImplementedBy [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the SSRow entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsImplementedBy(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsImplementedBy: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
