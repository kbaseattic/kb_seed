...
er-query-entity-Role

=head1 SYNOPSIS

er-query-entity-Role [--is field,value] [--like field,value] [--op operator,field,value]
...
Example:
    ...
    er-query-entity-Role -is id,exact-match-value -a > records
...
er-query-entity-Role [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Role: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
