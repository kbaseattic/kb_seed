...
er-get-relationship-IsDefinedBy

=head1 SYNOPSIS

er-get-relationship-IsDefinedBy [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-IsDefinedBy -a < ids > table.with.fields.added
...
Usage: er-get-relationship-IsDefinedBy [arguments] < ids > table.with.fields.added
...
my @all_to_fields = ( 'id', 'name', 'description', 'unitOfMeasure', 'category', 'source_id' );
...
our $usage = <<'END';
Usage: get-relationship-IsDefinedBy [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the MeasurementDescription entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsDefinedBy(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-IsDefinedBy: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
