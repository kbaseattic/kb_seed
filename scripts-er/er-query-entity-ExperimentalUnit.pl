...
er-query-entity-ExperimentalUnit

=head1 SYNOPSIS

er-query-entity-ExperimentalUnit [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity ExperimentalUnit. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-ExperimentalUnit -is id,exact-match-value -a > records
...
er-query-entity-ExperimentalUnit [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-ExperimentalUnit: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_ExperimentalUnit(\@qry, \@fields );
