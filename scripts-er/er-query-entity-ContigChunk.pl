...
er-query-entity-ContigChunk

=head1 SYNOPSIS

er-query-entity-ContigChunk [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity ContigChunk. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-ContigChunk -is id,exact-match-value -a > records
...
er-query-entity-ContigChunk [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-ContigChunk: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
