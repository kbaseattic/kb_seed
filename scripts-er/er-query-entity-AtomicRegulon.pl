...
er-query-entity-AtomicRegulon

=head1 SYNOPSIS

er-query-entity-AtomicRegulon [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity AtomicRegulon. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-AtomicRegulon -is id,exact-match-value -a > records
...
er-query-entity-AtomicRegulon [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-AtomicRegulon: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
