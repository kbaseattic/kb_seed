...
er-query-entity-LocationInstance

=head1 SYNOPSIS

er-query-entity-LocationInstance [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity LocationInstance. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-LocationInstance -is id,exact-match-value -a > records
...
er-query-entity-LocationInstance [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-LocationInstance: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_LocationInstance(\@qry, \@fields );
