...
er-query-entity-OTU [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity OTU. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-OTU -is id,exact-match-value -a > records
...
er-query-entity-OTU [arguments] > records
...
my $h = $geO->query_entity_OTU(\@qry, \@fields );
