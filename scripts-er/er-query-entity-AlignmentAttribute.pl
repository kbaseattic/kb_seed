...
er-query-entity-AlignmentAttribute

=head1 SYNOPSIS

er-query-entity-AlignmentAttribute [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity AlignmentAttribute. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-AlignmentAttribute -is id,exact-match-value -a > records
...
er-query-entity-AlignmentAttribute [arguments] > records
...
my $h = $geO->query_entity_AlignmentAttribute(\@qry, \@fields );
