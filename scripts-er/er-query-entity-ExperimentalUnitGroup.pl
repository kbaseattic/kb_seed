...
er-query-entity-ExperimentalUnitGroup

=head1 SYNOPSIS

er-query-entity-ExperimentalUnitGroup [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity ExperimentalUnitGroup. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-ExperimentalUnitGroup -is id,exact-match-value -a > records
...
er-query-entity-ExperimentalUnitGroup [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-ExperimentalUnitGroup: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_ExperimentalUnitGroup(\@qry, \@fields );
