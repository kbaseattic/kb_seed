...
er-get-relationship-Summarizes

=head1 SYNOPSIS

er-get-relationship-Summarizes [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-Summarizes -a < ids > table.with.fields.added
...
Usage: er-get-relationship-Summarizes [arguments] < ids > table.with.fields.added
...
my @all_from_fields = ( 'id', 'source_id', 'position', 'minor_AF', 'minor_allele', 'major_AF', 'major_allele', 'obs_unit_count' );
...
our $usage = <<'END';
Usage: get-relationship-Summarizes [arguments] < ids > table.with.fields.added
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_Summarizes(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-Summarizes: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
