...
er-get-relationship-Determines

=head1 SYNOPSIS

er-get-relationship-Determines [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    er-get-relationship-Determines -a < ids > table.with.fields.added
...
Usage: er-get-relationship-Determines [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the Pairing
...
my @all_rel_fields = ( 'from_link', 'to_link', 'inverted' );
my @all_to_fields = ( 'id', 'score' );
...
our $usage = <<'END';
Usage: get-relationship-Determines [arguments] < ids > table.with.fields.added
...
--to field-list
    Choose a set of fields from the PairSet entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_Determines(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "er-get-relationship-Determines: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
