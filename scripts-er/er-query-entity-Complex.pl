...
er-query-entity-Complex

=head1 SYNOPSIS

er-query-entity-Complex [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Complex. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Complex -is id,exact-match-value -a > records
...
er-query-entity-Complex [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Complex: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Complex(\@qry, \@fields );
