...
er-query-entity-CodonUsage

=head1 SYNOPSIS

er-query-entity-CodonUsage [--is field,value] [--like field,value] [--op operator,field,value]
...
Example:
    ...
    er-query-entity-CodonUsage -is id,exact-match-value -a > records
...
er-query-entity-CodonUsage [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-CodonUsage: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
