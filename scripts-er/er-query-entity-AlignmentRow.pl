...
er-query-entity-AlignmentRow

=head1 SYNOPSIS

er-query-entity-AlignmentRow [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity AlignmentRow. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-AlignmentRow -is id,exact-match-value -a > records
...
er-query-entity-AlignmentRow [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-AlignmentRow: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_AlignmentRow(\@qry, \@fields );
