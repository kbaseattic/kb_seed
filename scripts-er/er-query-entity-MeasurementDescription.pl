...
er-query-entity-MeasurementDescription

=head1 SYNOPSIS

er-query-entity-MeasurementDescription [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity MeasurementDescription. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-MeasurementDescription -is id,exact-match-value -a > records
...
er-query-entity-MeasurementDescription [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-MeasurementDescription: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_MeasurementDescription(\@qry, \@fields );
