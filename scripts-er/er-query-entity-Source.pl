...
er-query-entity-Source

=head1 SYNOPSIS

er-query-entity-Source [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Source. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Source -is id,exact-match-value -a > records
...
er-query-entity-Source [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Source: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Source(\@qry, \@fields );
