...
er-query-entity-PairSet

=head1 SYNOPSIS

er-query-entity-PairSet [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity PairSet. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-PairSet -is id,exact-match-value -a > records
...
er-query-entity-PairSet [arguments] > records
...
my $h = $geO->query_entity_PairSet(\@qry, \@fields );
