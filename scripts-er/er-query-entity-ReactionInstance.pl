...
er-query-entity-ReactionInstance

=head1 SYNOPSIS

er-query-entity-ReactionInstance [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity ReactionInstance. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-ReactionInstance -is id,exact-match-value -a > records
...
er-query-entity-ReactionInstance [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-ReactionInstance: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_ReactionInstance(\@qry, \@fields );
