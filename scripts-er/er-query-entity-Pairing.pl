...
er-query-entity-Pairing

=head1 SYNOPSIS

er-query-entity-Pairing [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Pairing. Results are limited using one or more of the query flags:
...
Example:
    ...
    er-query-entity-Pairing -is id,exact-match-value -a > records
...
er-query-entity-Pairing [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "er-all-entities-Pairing: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
