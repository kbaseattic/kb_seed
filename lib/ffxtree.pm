...
#     anno_vectors  => [\%va1, \%va2 ...]     # use popup menus to show annotation vectors
...
#                                               'genus' (D), 'species', 'strain', a hash, or 0
...
sub fix_tip_color_by {
    ...
    if ($by =~ /tax/i && $opts->{tax}) {
        $opts->{color_by} = { map { $_ => $opts->{tax}->{$_} ? major_taxonomy_group($opts->{tax}->{$_}) : major_taxonomy_group($opts->{tax}->{$opts->{fig}->{$_}}) } @tips };
...
sub assign_tip_colors {
    ...
    my %topc = map  { $_ && shift @colors ? ($_ => $c++) : () } @topf;

    $opts->{color} ||= { map { $_ => $topc{$by->{$_}} } newick_tip_list($tree) };

    $opts->{color_legend} ||= \%topc;
    $opts->{color_for_role} = \%topc;

    # if ($opts->{color_for_role}) {
    #     $opts->{color_for_role} = { map { my $c = shift @colors; $c ? ($_ => $c) : () } @topf };
    # }
}
...
sub fix_tip_collapse_by {
    ...
    my $nword = ($by =~ /g/i) ? 1 : ($by =~ /strain/ ? 100 : 2); # genus or species
...
sub fix_tip_annotation_vectors {
    my ($tree, $opts) = @_;

    for my $md5 (newick_tip_list($tree)) {
        for my $i (0..$#{$opts->{anno_vectors}}) {
            my $vec = $opts->{anno_vectors}->[$i]->{$md5} or next;
            my $maxscr;
            my $mainrole;
            for my $role (keys %$vec) {
                if ($vec->{$role}->[0] > $maxscr) {
                    $maxscr = $vec->{$role}->[0];
                    $mainrole = $role;
                }
            }
            
            my $bar  = score_to_vbar($maxscr);
            $bar = span_css($bar, $opts->{color_for_role}->{$mainrole}) if $opts->{color_for_role};

            my $html = annotation_vector_to_html($vec, $opts->{color_for_role});
            my $menu = span_mouseover($bar, 'Annotation', $html, undef, undef, undef, "#ffffff");

            # my $tip = gjoalign2html::mouseover('test', undef, $html, 1);
            
            $opts->{anno_html}->{$md5}->[$i] = $menu;
        }
    }
}

sub annotation_vector_to_html {
    my ($va, $colors) = @_;
    return unless $va && %$va;
    my @html;

    push @html, "<table border=0 cellpadding=2 cellspacing=1>";
    push @html, "<tr>";
    # push @html, "<th></th><th>Score</th> <th>Curator</th> <th width=\"70%\">Role</th> <th>Source</th>";
    push @html, "<th>$_</th>" for ('', qw(Score Curator Role Source));
    push @html, "</tr>";

    for my $role (sort { $va->{$a} <=> $va->{$b} } keys %$va) {
        $va->{$role}->[2] = span_css($va->{$role}->[2], $colors->{$va->{$role}->[2]});
        
        push @html, "<tr>";
        my $hbar = score_to_hbar($va->{$role}->[0]);
        # push @html, "<td>$_</td>" for ($hbar, @{$va->{$role}});
        push @html, "<td><nobr>$_</nobr></td>" for ($hbar, @{$va->{$role}});
        push @html, "</tr>";
    }
    ...
    # for my $role (sort { $va->{$a} <=> $va->{$b} } keys %$va) {
    #     my $hbar = score_to_hbar($va->{$role}->[0]);
    #     push @html, sprintf("<nobr>%s  %.4f  %20s  %s  %s</nobr><br>", $hbar, @{$va->{$role}});
    # }
...
sub html_tree_body {
    ...
    my $treecopy = gjonewicklib::copy_newick_tree($tree);
    ...
    unless ($opts->{raw}) {
        ...
        $tree = gjonewicklib::copy_newick_tree($treecopy) unless $tree;
    }

    fix_branch_support_values($tree, $opts) if $opts->{show_branch};

    $treecopy = gjonewicklib::copy_newick_tree($tree);
    fix_tip_collapse_by($tree, $opts);
    $tree = collapse_tree_by_group($tree, $opts);
    $tree = gjonewicklib::copy_newick_tree($treecopy) unless $tree;
    ...
    fix_tip_annotation_vectors($tree, $opts);
    ...
    for (gjonewicklib::newick_tip_list($tree)) { 
        ...
        $anno  = span_annotation_tables($id, $opts->{anno_html}->{$_}) if $opts->{anno_html}->{$_};

        # my $new = join(" ", $id, $desc, $ident, $subt, $anno, $text);
        my $new = join(" ", $anno, $id, $desc, $ident, $subt, $text);
        $relabel->{$_} = $new;
    }
...
sub span_mouseover {
    ...
    my $tip = gjoalign2html::mouseover($title, $html, $menu, $parent, $titlecolor, $bodycolor);
...
sub read_vector_annotation {
    my ($file) = @_;
    return undef unless -s $file;

    my $anno;
    
    my @lines = split(/\n/, SeedAware::slurp_input($file));
    for (@lines) {
        my ($md5, $role, $score, $curator, $source) = split /\t/;
        $score = sprintf("%.4f", $score);
        $anno->{$md5}->{$role} = [ $score, $curator, $role, $source ];
    }
