...
=head3 update_tracks($fc)

Update our tracks given a FeatureContext object containing
the data to render.
...
sub update_tracks
{
    my($self, $fc, $color_map, $group, $group_count) = @_;

    my %pin = map { $_->fid => 1 } @{$fc->pin}, $fc->focus;
    ...
    #my $pin_direction = '+';
    my $pin_direction = $fc->focus->dir;
    my $focus_peg = $fc->focus->fid;
    ...
    my $txt_max = 0;
    my $is_focus_row = 1;
    for my $genome ($fc->genomes)
    {
	my($row) = $fc->get_genome_context($genome);
	if (@$row == 0)
	{
	...
	#
	# Hook in here to shorten genome names.
	#
	...
	my $track = Track->new(panel => $self, genome => $genome);
	...
	{
	    my $peg = $fobj->fid;
	    my ($contig, $min, $max, $dir) = @{$fobj->boundaries};
	    my $func = $fobj->function;
	    my $group = $color_map->{$peg};
