...
#    hash   => bool          # return the line and layout hash
#    min_dx => whole_number  # the minimum horizontal branch length
#    width  => whole_number  # approximate tree width without labels
#
#  Character sets:
...
sub text_plot_newick
{
    ...
    $min_dx = int( $min_dx );
    $dy     = int( $dy );
    my $tree_max_x = newick_max_X( $node ) || 1;  # Div by zero caught by RAE
    my $x_scale = $width / $tree_max_x;
    ...
    foreach my $y ( ( $y1 .. $y2 ) )
    {
        my $line = text_tree_row( $node, $hash, $y, [], 'tee_l', $dy >= 2 );
        my $lbl  = '';
        if ( @$line && $line->[-1] eq '' ) { pop @$line; $lbl = pop @$line }
        push @lines, [ $line, $lbl ];
    }

    if ( $opts->{ scale_bar } || $opts->{ scale_lbl } )
    {
        my $bar_targ = 0.4 * $tree_max_x;
        my $bar_log  = int( log( $bar_targ ) / log( 10 ) + 100 ) - 100;
        my $bar_len  = 10**$bar_log;
        my $digit    = 5 * $bar_len <= $bar_targ ? 5 :
                       2 * $bar_len <= $bar_targ ? 2 : 1;
        $bar_len    *= $digit;
        my $bar_dx   = int( $x_scale * $bar_len );

        my $prefix   = $bar_log < 0 ? '0.' . ( '0' x ( -($bar_log+1) ) ) : '';
        my $suffix   = $bar_log > 0 ?          '0' x     $bar_log        : '';
        my $bar_lbl  = $prefix . $digit . $suffix;
        $bar_lbl    .= ' ' . $opts->{ scale_lbl } if $opts->{ scale_lbl };
        my $lbl_len  = length( $bar_lbl );

        my $bar_pad  = 0;
        my $lbl_pad  = int( 0.5 * ( $bar_dx - $lbl_len ) + 1001 ) - 1000;
        if ( $lbl_pad < 0 ) { $bar_pad = - $lbl_pad; $lbl_pad = 0; }
        $bar_dx     -= 1 if $bar_dx > 0;

        my @lbl = ( ('space') x $lbl_pad, split( //, $bar_lbl ) );
        my @bar = ( ('space') x $bar_pad, 'tee_r', ( 'horiz' ) x $bar_dx, 'tee_l' );
        my $nreq = ( @lbl > @bar ? @lbl : @bar ) + 3;

        #  See if we can nest the bar into the bottom or top of the tree:

        if ( @lines > 3 && ( @{ $lines[-3]->[0] } > $nreq && is_space( @{ $lines[-3]->[0] }[0..$nreq-1] ) )
                        && ( @{ $lines[-2]->[0] } > $nreq && is_space( @{ $lines[-2]->[0] }[0..$nreq-1] ) )
                        && ( @{ $lines[-2]->[0] } > $nreq && is_space( @{ $lines[-2]->[0] }[0..$nreq-1] ) )
           )
        {
            splice @{ $lines[-2]->[0] }, 0, @lbl, @lbl;
            splice @{ $lines[-1]->[0] }, 0, @bar, @bar;
        }
        elsif ( @lines > 3 && ( @{ $lines[2]->[0] } > $nreq && is_space( @{ $lines[2]->[0] }[0..$nreq-1] ) )
                           && ( @{ $lines[1]->[0] } > $nreq && is_space( @{ $lines[1]->[0] }[0..$nreq-1] ) )
                           && ( @{ $lines[0]->[0] } > $nreq && is_space( @{ $lines[0]->[0] }[0..$nreq-1] ) )
              )
        {
            splice @{ $lines[1]->[0] }, 0, @lbl, @lbl;
            splice @{ $lines[0]->[0] }, 0, @bar, @bar;
        }
        else
        {
            push @lines, [ \@lbl, '' ], [ \@bar, '' ];
    ...
    @lines = map { my ( $symbs, $lbl ) = @$_;
                   #  Translate tree graphic symbols
                   @$symbs = map { $charH->{ $_ } || $_ } @$symbs if $chars ne 'raw';
                   # Convert to requested output format:
                   $fmt eq 'text'         ?   join( '', @$symbs, ( $lbl ? " $lbl" : () ) )
                 : $fmt eq 'text_tab_lbl' ?   join( '', @$symbs, "\t", $lbl )
                 : $fmt eq 'tree_lbl'     ? [ join( '', @$symbs ), $lbl ]
                 : $fmt eq 'chrlist_lbl'  ?   $_
                 :                            ()
                 }
            @lines;

    wantarray ? ( $opts->{hash} ? ( \@lines, $hash ) : @lines ) : \@lines;
}


sub is_space { foreach ( @_ ) { return 0 if $_ ne 'space' } return 1 } 


sub node_row_col
{
    my ( $node, $hash ) = @_;
    return ( undef, undef ) unless $node && $hash && $hash->{$node};
    my ( $y1, undef, undef, $x, $y ) = @{ $hash->{$node} };
    ( $x, $y );
}
