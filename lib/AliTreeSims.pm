...
sub distance_based_sim
{
    ...
    my $eff_qlen = ( $npos1 > 60 ) ? $npos1 - 30 : 30;
    my $evalue   = $eff_qlen * $dbsize * exp( -$bitscr * $log2 );
...
sub md5_tree_sims_of_align
{
    ...
    my $to_distH = gjonewicklib::newick_distances_from_tip( $md5_tree, $id1 );
...
sub merged_sims
{
    ...
    @sims = grep { $_->psc             <= $max_e_value           } @sims;
    ...
    foreach ( @sims ) { $_->[10] =~ s/e-0(\d\d)$/e-$1/ }
