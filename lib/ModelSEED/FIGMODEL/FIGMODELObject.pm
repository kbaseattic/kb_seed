...
sub new {
	...
	my $self = {};
	bless $self;
	$args = $self->process_arguments($args,["filename"],{delimiter => "\t",headings => [],-load => 0});
	if (defined($args->{error})) {
		$self->error_message($args->{error});
		return undef;
	}
...
=head3 error_message
Definition:
	string = FIGMODELObject->error_message(string:message);
Description:
    Prints and returns the input message
=cut
sub error_message {
    my ($self,$error) = @_;
    $error = "FIGMODELObject:".$error;
    print STDERR $error."\n";
    return $error;
}
