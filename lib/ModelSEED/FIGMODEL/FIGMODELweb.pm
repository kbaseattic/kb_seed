...
sub display_reaction_enzymes {
	...
	$enzymes =~ s/\|/,/g;
	return substr($enzymes,1,length($enzymes)-2);
}
...
Definition:
	string = FIGMODELweb->table_model_column({object => string:PPOobject,
											  function => string:PPOobject function,
											  type => string:alias type,
											  -delimiter => string:delimiter for output});
Description:
...
sub table_model_column {
	...
	if (!defined($model)) {
		return $args->{model}." MODEL NOT FOUND";
	}
	#Generating output based on the object type
	if ($args->{type} eq "compound") {
		my $output = "";
		my $cpd = $model->get_compound_data($args->{data});
		if (!defined($cpd)) {
			return "Not in model";
		}
		...
		my $rxn = $model->get_reaction_data($args->{data});
		if (!defined($rxn)) {
			return "Not in model";
		}
		#Getting the reaction class
		my $output;
		my $rxnclass = $model->get_reaction_class($args->{data});
		if (defined($rxnclass)) {
			$output = $rxnclass."<br>";
		}
		#Handling genes
		...
		if ($model->source() !~ m/^MGRAST/) {
			...
			$output .= $self->figmodel()->ParseForLinks($PegString,$args->{model});
			$output =~ s/\(\s/(/g;
			$output =~ s/\s\)/)/g;
			return $output;
		}
...
sub create_sbml_link {
	...
	my $Link = '<a  href="ModelSEEDdownload.cgi?model='.$model->id().'&file='.$type.'" title="'.$title.'">'.$name.'</a>';
	return $Link;
}
