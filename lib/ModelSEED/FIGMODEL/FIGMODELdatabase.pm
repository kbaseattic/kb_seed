...
sub get_object_manager {
	...
	if (!defined($self->{_dbhandles}->{$self->config("PPO_tbl_".$type)->{name}->[0]}) || !$self->{_dbhandles}->{$self->config("PPO_tbl_".$type)->{name}->[0]}->db_handle()->ping) {
		if (defined($self->{_dbhandles}->{$self->config("PPO_tbl_".$type)->{name}->[0]})) {
			$self->figmodel()->error_message("Database connection with ".$self->config("PPO_tbl_".$type)->{name}->[0]." lost.");
			$self->{_dbhandles}->{$self->config("PPO_tbl_".$type)->{name}->[0]}->DESTROY();
		}
    	...
    	$self->{_dbhandles}->{$self->config("PPO_tbl_".$type)->{name}->[0]} = $temp;
	}
	my $table = $self->config("PPO_tbl_".$type)->{table}->[0];
	return $self->{_dbhandles}->{$self->config("PPO_tbl_".$type)->{name}->[0]}->$table();
}
...
sub get_objects {
	...
	if (defined($objs->[0]) && defined($self->figmodel()->config("objects with rights")->{$type})) {
		...
		if (defined($self->figmodel()->config("model administrators")->{$self->figmodel()->user()})) {
			$admin = 1;
		} elsif ($self->figmodel()->user() ne "PUBLIC" && defined($self->figmodel()->userObj())) {
			if (!defined($self->getCache($type.":_rights:"))) {
				my $user_scopes = $self->get_objects("userscope", {user => $self->figmodel()->userObj()});
				my $rights = $self->get_object_hash({type => "right",
                    attribute => sub { my $x = shift; return $x->scope()->_id(); },
                    -useCache => 1,
                    parameters => {data_type => $type}
				});
				for (my $i=0; $i < @{$user_scopes}; $i++) {
                    my $scope_id = $user_scopes->[$i]->scope()->_id();
					if (defined($rights->{$scope_id})) {
						for (my $j=0; $j < @{$rights->{$scope_id}}; $j++) {
							$rightsHash->{$rights->{$scope_id}->[$j]->data_id()} = 1;
						}
					}
				}
				$self->setCache($type.":_rights:",$rightsHash);
			} else {
				$rightsHash = $self->getCache($type.":_rights:");
			}
		}
		my $finalObjs;
		for (my $i=0; $i < @{$objs}; $i++) {
			if ($objs->[$i]->public() == 1 || $admin == 1 || defined($rightsHash->{$objs->[$i]->_id()})) {
...
sub get_object_hash {
	...
	my $cacheBehavior = 0;
	if ($args->{-onlyIfCached} == 1) {
		$cacheBehavior = 3;
	} elsif ($args->{-useCache} == 1) {
		$cacheBehavior = 1;
	}
	my $hash;
	my $cacheKey;
...
sub get_object_rights {
    ...
    if(defined($self->figmodel()->config("model administrators")->{$login})) {
        return {'admin' => 1};
    } elsif(not defined($self->figmodel()->config("objects with rights")->{$type})) {
        return {'view' => 1};
    } elsif($login ne "PUBLIC" && defined($user)) {
        my $scopes = $self->get_objects("userscope", {user => $user});
        my %scopeHash = map { $_->scope()->_id() => 1 } @$scopes;
        my $rights = $self->get_objects("right", {data_type => $type, data_id => $object->_id()});
        my $roleHash = {};
        foreach my $right (@$rights) {
            if(defined($scopeHash{$right->scope()->_id()})) {
                $roleHash->{$right->name()} = 1;
            }
        }
        return $roleHash;
    } elsif (defined($object->attributes()->{public}) && $object->public() eq 1) {
        return {'view' => 1};
    } else {
        return {};
    }
...
sub build_link_file {
	...
	my $Table = ModelSEED::FIGMODEL::FIGMODELTable->new([$EntityOne,$EntityTwo,"DATA"],$self->config("Reaction database directory")->[0]."linkfiles/".$EntityOne."-".$EntityTwo.".tbl",[$EntityOne,$EntityTwo],"\t","|",undef);
...
sub interval_model_media {
	...
	my $tbl;
	if (!-e $self->figmodel()->directory()."ReactionDB/linkfiles/".$id.".tbl") {
		$tbl = ModelSEED::FIGMODEL::FIGMODELTable->new($headings,$self->figmodel()->directory()."ReactionDB/linkfiles/".$id.".tbl",$entityList,"\t","|",undef);
	} else {
...
sub get_genome_feature_table {
	...
	my $featuretbl = new ModelSEED::FIGMODEL::FIGMODELTable(["ID","ALIASES","TYPE","LOCATION","MIN LOCATION","MAX LOCATION","ROLES","SEQUENCE"],$names->[0]."Features-".$OrganismID.".txt",["ID","ALIASES","TYPE","ROLES"],"\t","|",undef);
	...
	for (my $i=0; $i < @{$types}; $i++) {
		...
		if (@entities > 0 && ($types->[$i] eq "Lipid" || $types->[$i] eq "CellWall" || $types->[$i] eq "Cofactor" || $types->[$i] eq "Unknown")) {
			...
			if (defined($packageHash->{join("|",@entities)})) {
				...
				$package->{$types->[$i]} = $self->check_out_new_id($types->[$i]."Pkg");
...
sub ppo_rows_to_table {
    ...
    my $table = ModelSEED::FIGMODEL::FIGMODELTable->new(
        $config->{'headings'}, $config->{'filename'},
        $config->{'hash_headings'}, $config->{'delimiter'},
        $config->{'item_delimiter'}, $config->{'prefix'});
    # Now add the rows, testing if the row object has each attribute
    foreach my $row (@$rows) {
        my %rowHashList = map { $_ => [] } @{$config->{'headings'}};
        foreach my $key (keys %rowHashList) {
            if($row->_knows_attribute($key)) {
               push(@{$rowHashList{$key}}, $row->$key());
            }
        }
        $table->add_row(\%rowHashList);
    }
    return $table;
}
