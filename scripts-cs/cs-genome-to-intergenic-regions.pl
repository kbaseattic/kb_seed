...
cs-genome-to-intergenic-regions

=head1 SYNOPSIS

cs-genome-to-intergenic-regions -g GenomeID [-pad=num] > output

=head1 DESCRIPTION

cs-genome-to-intergenic-regions is used to get the intergenic regions in a specified
...
Example:
    ...
    cs-genome-to-intergenic-regions -g 'kb|g.0' -pad=10 > output
...
Usage: cs-genome-to-intergenic-regions -g GenomeID [-pad=num] > output
...
our $usage = "usage: cs-genome-to-intergenic-regions > output";
...
open(LOCS,"echo 'kb|g.0' | cs-genomes-to-fids peg CDS rna | er-get-relationship-IsLocatedIn -to id -rel begin,dir,len | cut -f2,3,4,5,6 | sort -k 5 -k 2n |")
