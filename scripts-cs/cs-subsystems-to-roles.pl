...
cs-subsystems-to-roles

=head1 SYNOPSIS

cs-subsystems-to-roles [arguments] < input > output

=head1 DESCRIPTION

The command cs-subsystems-to-roles takes as input a table with a column containing subsystem IDs.  The table is
...
Usage: cs-subsystems-to-roles [arguments] < input > output
...
our $usage = "usage: cs-subsystems-to-roles [-c column] < input > output";
