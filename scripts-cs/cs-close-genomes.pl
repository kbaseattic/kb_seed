...
cs-close-genomes [arguments] < input > output
...
our $usage = "usage: cs-close-genomes [-c column] < input > output";
...
else
{
	...
	{
	    my ($g, $line) = @$tuple;
	    open(CONTIGS,"echo '$g' | cs-genomes-to-contigs | cs-contigs-to-sequences |");
