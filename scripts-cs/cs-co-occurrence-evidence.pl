...
cs-co-occurrence-evidence

=head1 SYNOPSIS

cs-co-occurrence-evidence [-c column] < input > output

=head1 DESCRIPTION

cs-co-occurence-evidence is used to retrieve the detailed pairs of genes that go into the
...
co-occurring pairs.  For example,
     ...
     echo 'kb|g.0.peg.101:kb|g.0.peg.263' | cs-co-occurrence-evidence 
...
Usage: cs-co-occurrence-evidence [-c column] < input > output
...
our $usage = "usage: cs-co-occurrence-evidence [-c column] < input > output";
