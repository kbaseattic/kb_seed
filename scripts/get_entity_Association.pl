...
get_entity_Association [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    get_entity_Association -a < ids > table.with.fields.added
...
Usage: get_entity_Association [arguments] < ids > table.with.fields.added
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "get_entity_Association: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
} else {
    print STDERR $usage;
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_entity_Association(\@h, \@fields);
    for my $tuple (@tuples) {
        my @values;
        my ($id, $line) = @$tuple;
        my $v = $h->{$id};
