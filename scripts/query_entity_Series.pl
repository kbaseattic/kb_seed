...
query_entity_Series [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Series. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_Series -is id,exact-match-value -a > records
...
query_entity_Series [arguments] > records
...
my $h = $geO->query_entity_Series(\@qry, \@fields );
