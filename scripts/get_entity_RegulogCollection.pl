...
get_entity_RegulogCollection

=head1 SYNOPSIS

get_entity_RegulogCollection [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    get_entity_RegulogCollection -a < ids > table.with.fields.added
...
Usage: get_entity_RegulogCollection [arguments] < ids > table.with.fields.added
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "get_entity_RegulogCollection: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
} else {
    print STDERR $usage;
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_entity_RegulogCollection(\@h, \@fields);
    for my $tuple (@tuples) {
        my @values;
        my ($id, $line) = @$tuple;
        my $v = $h->{$id};
