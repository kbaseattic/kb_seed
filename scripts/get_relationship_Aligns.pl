...
my @all_from_fields = ( 'id', 'alignment_method', 'alignment_parameters', 'alignment_properties', 'tree_method', 'tree_parameters', 'tree_properties' );
...
my $usage = "usage: get_relationship_Aligns [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)";
...
while (my @tuples = ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_Aligns(\@h, \@from_fields, \@rel_fields, \@to_fields); 
    for my $result (@$h) {
        my @from;
        my @rel;
        my @to;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_Aligns: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
