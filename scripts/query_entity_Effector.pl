...
query_entity_Effector

=head1 SYNOPSIS

query_entity_Effector [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Effector. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_Effector -is id,exact-match-value -a > records
...
query_entity_Effector [arguments] > records
...
my $h = $geO->query_entity_Effector(\@qry, \@fields );
