...
get_relationship_FeatureMeasuredBy

=head1 SYNOPSIS

get_relationship_FeatureMeasuredBy [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    get_relationship_FeatureMeasuredBy -a < ids > table.with.fields.added
...
Usage: get_relationship_FeatureMeasuredBy [arguments] < ids > table.with.fields.added
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_FeatureMeasuredBy: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
