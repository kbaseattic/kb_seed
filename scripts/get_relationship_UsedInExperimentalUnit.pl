...
=head1 get_relationship_UsedInExperimentalUnit
...
Example:
    ...
    get_relationship_UsedInExperimentalUnit -a < ids > table.with.fields.added
...
Choose a set of fields from the Environment entity to return. Field-list is a comma-separated list of 
...
my @all_from_fields = ( 'id', 'temperature', 'description', 'anaerobic', 'pH', 'source_id' );
...
my $usage = "usage: get_relationship_UsedInExperimentalUnit [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)\n";
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_UsedInExperimentalUnit(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_UsedInExperimentalUnit: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
