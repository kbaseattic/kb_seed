...
query_entity_RegulogCollection

=head1 SYNOPSIS

query_entity_RegulogCollection [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity RegulogCollection. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_RegulogCollection -is id,exact-match-value -a > records
...
query_entity_RegulogCollection [arguments] > records
...
my $h = $geO->query_entity_RegulogCollection(\@qry, \@fields );
