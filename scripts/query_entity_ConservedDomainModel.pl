...
query_entity_ConservedDomainModel

=head1 SYNOPSIS

query_entity_ConservedDomainModel [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity ConservedDomainModel. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_ConservedDomainModel -is id,exact-match-value -a > records
...
my $h = $geO->query_entity_ConservedDomainModel(\@qry, \@fields );
