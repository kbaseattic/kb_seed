...
=head1 get_entity_LocationInstance
...
Example:
    ...
    get_entity_LocationInstance -a < ids > table.with.fields.added
...
my $usage = "usage: get_entity_LocationInstance [-h] [-c column] [-a | -f field list] < ids > extended.by.a.column(s)";
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "get_entity_LocationInstance: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
} else {
    print STDERR $usage;
