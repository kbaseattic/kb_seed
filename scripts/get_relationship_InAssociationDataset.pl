...
get_relationship_InAssociationDataset

=head1 SYNOPSIS

get_relationship_InAssociationDataset [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    get_relationship_InAssociationDataset -a < ids > table.with.fields.added
...
Usage: get_relationship_InAssociationDataset [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the Association
...
Choose a set of fields from the AssociationDataset entity to return. Field-list is a comma-separated list of 
strings. The following fields are available:
...
my @all_from_fields = ( 'id', 'name', 'description', 'directional', 'confidence', 'url' );
my @all_rel_fields = ( 'from_link', 'to_link',  );
my @all_to_fields = ( 'id', 'description', 'data_source', 'url', 'association_type' );
...
--to field-list
    Choose a set of fields from the AssociationDataset entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_InAssociationDataset(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_InAssociationDataset: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
