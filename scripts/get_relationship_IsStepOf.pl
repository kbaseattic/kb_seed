...
=head1 get_relationship_IsStepOf
...
Example:
    ...
    get_relationship_IsStepOf -a < ids > table.with.fields.added
...
my @all_to_fields = ( 'id', 'name', 'msid', 'mod_date' );
...
my $usage = "usage: get_relationship_IsStepOf [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)\n";
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_IsStepOf: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
