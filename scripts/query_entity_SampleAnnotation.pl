...
query_entity_SampleAnnotation

=head1 SYNOPSIS

query_entity_SampleAnnotation [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity SampleAnnotation. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_SampleAnnotation -is id,exact-match-value -a > records
...
query_entity_SampleAnnotation [arguments] > records
...
my $h = $geO->query_entity_SampleAnnotation(\@qry, \@fields );
