...
=head1 get_relationship_IsUsageOf

Example:

    get_relationship_IsUsageOf -a < ids > table.with.fields.added
...
my $usage = "usage: get_relationship_IsUsageOf [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)";
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_IsUsageOf: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
