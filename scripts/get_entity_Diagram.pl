...
get_entity_Diagram [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    get_entity_Diagram -a < ids > table.with.fields.added
...
Usage: get_entity_Diagram [arguments] < ids > table.with.fields.added
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "get_entity_Diagram: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
} else {
    print STDERR $usage;
