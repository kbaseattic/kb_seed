...
=head1 get_relationship_IsAlignedDNAComponentOf
...
Example:
    ...
    get_relationship_IsAlignedDNAComponentOf -a < ids > table.with.fields.added
...
Choose a set of fields from the ContigSequence entity to return. Field-list is a comma-separated list of 
...
my $usage = "usage: get_relationship_IsAlignedDNAComponentOf [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)\n";
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_IsAlignedDNAComponentOf: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
