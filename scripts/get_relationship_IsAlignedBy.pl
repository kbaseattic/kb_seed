...
=head1 get_relationship_IsAlignedBy
...
Example:
    ...
    get_relationship_IsAlignedBy -a < ids > table.with.fields.added
...
my @all_rel_fields = ( 'from_link', 'to_link', 'begin', 'end', 'len', 'sequence_id', 'properties' );
my @all_to_fields = ( 'id', 'alignment_method', 'alignment_parameters', 'alignment_properties', 'tree_method', 'tree_parameters', 'tree_properties' );
...
my $usage = "usage: get_relationship_IsAlignedBy [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)\n";
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsAlignedBy(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_IsAlignedBy: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
