...
query_entity_Sample [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Sample. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_Sample -is id,exact-match-value -a > records
...
query_entity_Sample [arguments] > records
...
my $h = $geO->query_entity_Sample(\@qry, \@fields );
