...
query_entity_Operon [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Operon. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_Operon -is id,exact-match-value -a > records
...
query_entity_Operon [arguments] > records
...
my $h = $geO->query_entity_Operon(\@qry, \@fields );
