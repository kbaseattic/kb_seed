...
query_entity_Regulog

=head1 SYNOPSIS

query_entity_Regulog [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Regulog. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_Regulog -is id,exact-match-value -a > records

=head2 Related entities

The Regulog entity has the following relationship links:

=over 4
    
=item HasRegulator Regulator

=item IsInCollection RegulogCollection

=item RegulogHasRegulon Regulon
...
query_entity_Regulog [arguments] > records
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "all_entities_Regulog: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_Regulog(\@qry, \@fields );
