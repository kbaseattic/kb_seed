...
get_relationship_IsRegulatorFor

=head1 SYNOPSIS

get_relationship_IsRegulatorFor [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    get_relationship_IsRegulatorFor -a < ids > table.with.fields.added
...
Usage: get_relationship_IsRegulatorFor [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the Regulog entity to return. Field-list is a comma-separated list of 
strings. The following fields are available:
...
--to field-list
    Choose a set of fields from the Regulog entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsRegulatorFor(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_IsRegulatorFor: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
