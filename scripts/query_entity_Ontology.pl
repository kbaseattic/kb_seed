...
query_entity_Ontology

=head1 SYNOPSIS

query_entity_Ontology [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Ontology. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_Ontology -is id,exact-match-value -a > records
...
query_entity_Ontology [arguments] > records
...
my $h = $geO->query_entity_Ontology(\@qry, \@fields );
