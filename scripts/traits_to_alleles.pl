...
=head1 traits_to_alleles

Example:

    traits_to_alleles [arguments] < input > output
...
This script is a wrapper for the CDMI-API call traits_to_alleles. It is documented as follows:
...
my $usage = "usage: traits_to_alleles [-c column] < input > output";
