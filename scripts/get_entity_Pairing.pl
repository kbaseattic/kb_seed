...
get_entity_Pairing [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Usage: get_entity_Pairing [arguments] < ids > table.with.fields.added
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "get_entity_Pairing: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
} else {
    print STDERR $usage;
