...
=head1 get_relationship_UsesAliasForCompound

Example:

    get_relationship_UsesAliasForCompound -a < ids > table.with.fields.added
...
my @all_from_fields = ( 'id', 'label', 'abbr', 'ubiquitous', 'mod_date', 'uncharged_formula', 'formula', 'mass' );
...
my $usage = "usage: get_relationship_UsesAliasForCompound [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)";
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_UsesAliasForCompound: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
