...
query_entity_Regulome

=head1 SYNOPSIS

query_entity_Regulome [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Regulome. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_Regulome -is id,exact-match-value -a > records
...
query_entity_Regulome [arguments] > records
...
my $h = $geO->query_entity_Regulome(\@qry, \@fields );
