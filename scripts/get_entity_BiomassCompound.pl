...
=head1 get_entity_BiomassCompound
...
Example:
    ...
    get_entity_BiomassCompound -a < ids > table.with.fields.added
...
my $usage = "usage: get_entity_BiomassCompound [-h] [-c column] [-a | -f field list] < ids > extended.by.a.column(s)";
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "get_entity_BiomassCompound: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
} else {
    print STDERR $usage;
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_entity_BiomassCompound(\@h, \@fields);
    for my $tuple (@tuples) {
        my @values;
        my ($id, $line) = @$tuple;
        my $v = $h->{$id};
