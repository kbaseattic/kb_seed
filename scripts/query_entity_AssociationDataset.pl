...
query_entity_AssociationDataset

=head1 SYNOPSIS

query_entity_AssociationDataset [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity AssociationDataset. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_AssociationDataset -is id,exact-match-value -a > records
...
query_entity_AssociationDataset [arguments] > records
...
my $h = $geO->query_entity_AssociationDataset(\@qry, \@fields );
