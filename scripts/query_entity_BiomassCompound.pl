...
=head1 query_entity_BiomassCompound

Query the entity BiomassCompound.
...
Example:
    ...
    query_entity_BiomassCompound -a 
...
my @all_fields = ( 'coefficient' );
my %all_fields = map { $_ => 1 } @all_fields, 'id';

my $usage = "usage: query_entity_BiomassCompound [-is field,value] [-like field,value] [-op operator,field,value] [-show-fields] [-a | -f field list] > entity.data";
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "all_entities_BiomassCompound: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
}
...
my $h = $geO->query_entity_BiomassCompound(\@qry, \@fields );
