...
query_entity_Platform

=head1 SYNOPSIS

query_entity_Platform [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Platform. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_Platform -is id,exact-match-value -a > records
...
query_entity_Platform [arguments] > records
...
my $h = $geO->query_entity_Platform(\@qry, \@fields );
