...
query_entity_Association

=head1 SYNOPSIS

query_entity_Association [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Association. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_Association -is id,exact-match-value -a > records
...
query_entity_Association [arguments] > records
...
my $h = $geO->query_entity_Association(\@qry, \@fields );
