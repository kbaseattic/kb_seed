...
=head1 get_relationship_HasAssertionFrom
...
Example:
    ...
    get_relationship_HasAssertionFrom -a < ids > table.with.fields.added
...
Choose a set of fields from the Identifier entity to return. Field-list is a comma-separated list of 
...
my @all_from_fields = ( 'id', 'source', 'natural_form' );
my @all_rel_fields = ( 'from_link', 'to_link', 'function', 'expert' );
my @all_to_fields = ( 'id',  );
...
my $usage = "usage: get_relationship_HasAssertionFrom [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)\n";
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_HasAssertionFrom(\@h, \@from_fields, \@rel_fields, \@to_fields); 
    for my $result (@$h) {
        my @from;
        my @rel;
        my @to;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_HasAssertionFrom: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
