...
get_relationship_SeriesWithSamples

=head1 SYNOPSIS

get_relationship_SeriesWithSamples [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    get_relationship_SeriesWithSamples -a < ids > table.with.fields.added
...
Usage: get_relationship_SeriesWithSamples [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the Series
...
my @all_from_fields = ( 'id', 'title', 'summary', 'design', 'externalSourceId', 'kbaseSubmissionDate', 'externalSourceDate', 'source_id' );
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_SeriesWithSamples(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_SeriesWithSamples: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
