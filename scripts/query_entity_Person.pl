...
=head1 query_entity_Person

Query the entity Person.

A person represents a human affiliated in some way with Kbase.

Example:

    query_entity_Person -a 
...
my $usage = "usage: query_entity_Person [-is field,value] [-like field,value] [-op operator,field,value] [-show-fields] [-a | -f field list] > entity.data";
