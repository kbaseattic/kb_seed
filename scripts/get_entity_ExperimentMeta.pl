...
=head1 get_entity_ExperimentMeta
...
Example:
    ...
    get_entity_ExperimentMeta -a < ids > table.with.fields.added
...
my $usage = "usage: get_entity_ExperimentMeta [-h] [-c column] [-a | -f field list] < ids > extended.by.a.column(s)";
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "get_entity_ExperimentMeta: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
} else {
    print STDERR $usage;
