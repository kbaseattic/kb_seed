...
get_relationship_DefaultControlSample

=head1 SYNOPSIS

get_relationship_DefaultControlSample [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    get_relationship_DefaultControlSample -a < ids > table.with.fields.added
...
Usage: get_relationship_DefaultControlSample [arguments] < ids > table.with.fields.added
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_DefaultControlSample(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_DefaultControlSample: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
