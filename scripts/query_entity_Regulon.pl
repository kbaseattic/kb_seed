...
query_entity_Regulon

=head1 SYNOPSIS

query_entity_Regulon [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Regulon. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_Regulon -is id,exact-match-value -a > records
...
query_entity_Regulon [arguments] > records
...
my $h = $geO->query_entity_Regulon(\@qry, \@fields );
