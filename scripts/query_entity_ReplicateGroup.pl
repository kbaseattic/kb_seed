...
query_entity_ReplicateGroup

=head1 SYNOPSIS

query_entity_ReplicateGroup [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity ReplicateGroup. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_ReplicateGroup -is id,exact-match-value -a > records
...
query_entity_ReplicateGroup [arguments] > records
...
my $h = $geO->query_entity_ReplicateGroup(\@qry, \@fields );
