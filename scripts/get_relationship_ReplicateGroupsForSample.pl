...
get_relationship_ReplicateGroupsForSample

=head1 SYNOPSIS

get_relationship_ReplicateGroupsForSample [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    get_relationship_ReplicateGroupsForSample -a < ids > table.with.fields.added
...
Usage: get_relationship_ReplicateGroupsForSample [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the ReplicateGroup
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_ReplicateGroupsForSample(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_ReplicateGroupsForSample: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
