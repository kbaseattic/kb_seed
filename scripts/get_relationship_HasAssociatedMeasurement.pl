...
=head1 get_relationship_HasAssociatedMeasurement
...
Example:
    ...
    get_relationship_HasAssociatedMeasurement -a < ids > table.with.fields.added
...
Choose a set of fields from the PhenotypeDescription entity to return. Field-list is a comma-separated list of 
...
my @all_from_fields = ( 'id', 'name', 'description', 'unitOfMeasure', 'source_id' );
my @all_rel_fields = ( 'from_link', 'to_link',  );
my @all_to_fields = ( 'id', 'timeSeries', 'source_id', 'value', 'mean', 'median', 'stddev', 'N', 'p_value', 'Z_score' );
...
my $usage = "usage: get_relationship_HasAssociatedMeasurement [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)\n";
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_HasAssociatedMeasurement(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_HasAssociatedMeasurement: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
