...
get_relationship_HasStep [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    get_relationship_HasStep -a < ids > table.with.fields.added
...
Usage: get_relationship_HasStep [arguments] < ids > table.with.fields.added
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_HasStep: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
