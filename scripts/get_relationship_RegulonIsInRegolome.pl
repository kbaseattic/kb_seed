...
get_relationship_RegulonIsInRegolome

=head1 SYNOPSIS

get_relationship_RegulonIsInRegolome [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Example:
    ...
    get_relationship_RegulonIsInRegolome -a < ids > table.with.fields.added
...
Usage: get_relationship_RegulonIsInRegolome [arguments] < ids > table.with.fields.added
...
Choose a set of fields from the Regulome entity to return. Field-list is a comma-separated list of 
strings. The following fields are available:
...
my @all_to_fields = ( 'id', 'description', 'creation_date' );
...
--to field-list
    Choose a set of fields from the Regulome entity to 
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_RegulonIsInRegolome(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_RegulonIsInRegolome: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
