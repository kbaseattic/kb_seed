...
get_relationship_IsTerminusFor [-c N] [-a] [--fields field-list] < ids > table.with.fields.added
...
Usage: get_relationship_IsTerminusFor [arguments] < ids > table.with.fields.added
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_IsTerminusFor: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
