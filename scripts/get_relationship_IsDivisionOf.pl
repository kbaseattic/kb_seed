...
=head1 get_relationship_IsDivisionOf
...
Example:
    ...
    get_relationship_IsDivisionOf -a < ids > table.with.fields.added
...
my @all_from_fields = ( 'id', 'compartment_index', 'label', 'pH', 'potential' );
...
my $usage = "usage: get_relationship_IsDivisionOf [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)\n";
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_IsDivisionOf: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
