...
=head1 get_relationship_IsARequirementIn
...
Example:
    ...
    get_relationship_IsARequirementIn -a < ids > table.with.fields.added
...
my @all_to_fields = ( 'id', 'direction', 'transproton', 'proton' );
...
my $usage = "usage: get_relationship_IsARequirementIn [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)\n";
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_IsARequirementIn(\@h, \@from_fields, \@rel_fields, \@to_fields);
    my %results;
    for my $result (@$h) {
        my @from;
        my @rel;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_IsARequirementIn: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
