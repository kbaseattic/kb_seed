...
query_entity_AssociationDetectionType

=head1 SYNOPSIS

query_entity_AssociationDetectionType [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity AssociationDetectionType. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_AssociationDetectionType -is id,exact-match-value -a > records
...
query_entity_AssociationDetectionType [arguments] > records
...
my $h = $geO->query_entity_AssociationDetectionType(\@qry, \@fields );
