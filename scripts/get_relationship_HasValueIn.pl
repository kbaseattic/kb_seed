...
=head1 get_relationship_HasValueIn
...
Example:
    ...
    get_relationship_HasValueIn -a < ids > table.with.fields.added
...
my $usage = "usage: get_relationship_HasValueIn [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)\n";
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_HasValueIn: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
