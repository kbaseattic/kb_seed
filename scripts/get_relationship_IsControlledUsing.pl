...
=head1 get_relationship_IsControlledUsing

Example:

    get_relationship_IsControlledUsing -a < ids > table.with.fields.added
...
my @all_from_fields = ( 'id', 'reason' );
my @all_rel_fields = ( 'from_link', 'to_link', 'effector' );
my @all_to_fields = ( 'id', 'feature_type', 'source_id', 'sequence_length', 'function' );
...
my $usage = "usage: get_relationship_IsControlledUsing [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)";
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_IsControlledUsing: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
