...
=head1 get_relationship_HasProposedLocationIn
...
Example:
    ...
    get_relationship_HasProposedLocationIn -a < ids > table.with.fields.added
...
my @all_from_fields = ( 'id', 'direction', 'transproton' );
...
my $usage = "usage: get_relationship_HasProposedLocationIn [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)\n";
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my @h = map { $_->[0] } @tuples;
    my $h = $geO->get_relationship_HasProposedLocationIn(\@h, \@from_fields, \@rel_fields, \@to_fields); 
    for my $result (@$h) {
        my @from;
        my @rel;
        my @to;
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_HasProposedLocationIn: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
