...
=head1 get_entity_Locality

A locality is a geographic location.

Example:

    get_entity_Locality -a < ids > table.with.fields.added
...
my $usage = "usage: get_entity_Locality [-h] [-c column] [-a | -f field list] < ids > extended.by.a.column(s)";
...
if ($a)
{
    ...
    if (@err)
    {
	print STDERR "get_entity_Locality: unknown fields @err. Valid fields are: @all_fields\n";
	exit 1;
    }
} else {
    print STDERR $usage;
