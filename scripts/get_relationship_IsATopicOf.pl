...
=head1 get_relationship_IsATopicOf
...
Example:
    ...
    get_relationship_IsATopicOf -a < ids > table.with.fields.added
...
my $usage = "usage: get_relationship_IsATopicOf [-c column] [-a | -from field list -rel field list -to field list] < ids > extended.by.a.column(s)\n";
...
sub check_fields {
	...
	if (@err) {
		...
		print STDERR "get_relationship_IsATopicOf: unknown fields @err. Valid fields are @f\n";
		return 1;
	}
	return 0;
}
