...
query_entity_Regulator

=head1 SYNOPSIS

query_entity_Regulator [--is field,value] [--like field,value] [--op operator,field,value]

=head1 DESCRIPTION

Query the entity Regulator. Results are limited using one or more of the query flags:
...
Example:
    ...
    query_entity_Regulator -is id,exact-match-value -a > records
...
query_entity_Regulator [arguments] > records
...
my $h = $geO->query_entity_Regulator(\@qry, \@fields );
